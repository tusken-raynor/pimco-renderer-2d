/**
 * Unit tests for VirtualTextSlave.
 */
...
describe('VirtualTextSlave', () => {
  ...
  beforeEach(() => {
    slave = new VirtualTextSlave({ deferMessages: false });
  });
  ...
  describe('deferred messages', () => {
    it('should defer messages when deferMessages option is true', async () => {
      const deferredSlave = new VirtualTextSlave({ deferMessages: true });
      const messages: SlaveToMasterMessage[] = [];
  ...
  describe('direct asset registration', () => {
    it('should accept direct image asset registration', () => {
    ...
    it('should accept direct font registration', () => {
      const fontData = new ArrayBuffer(10);

      // Should not throw
      expect(() => {
        slave.registerFontDirect(1, 'TestFont', fontData);
      }).not.toThrow();
    });
    ...
    it('should ignore asset registration after termination', () => {
      ...
      expect(() => {
        slave.registerFontDirect(2, 'TestFont', new ArrayBuffer(10));
      }).not.toThrow();
    });
  });
